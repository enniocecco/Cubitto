...
export default function TabOneScreen({ navigation, colorScheme }: { navigation: any, colorScheme: ColorSchemeName }) {
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <Appbar.Header theme={colorScheme === 'dark' ? DarkTheme : DefaultTheme} style={{  top: 0, backgroundColor: 'black', paddingTop:0 }}>
      <View style={{backgroundColor: 'black', top: 0, flexDirection: 'row', paddingTop:0}}>
            <Text style={{ marginLeft: 20, color: '#349cf4', fontSize: 26 }}>↑{clinetInfo.up_info_speed == null ? "0" : formatBytes(clinetInfo.up_info_speed)}/s</Text>
            
            <Text style={{ marginLeft: 20, color: '#39ce83', fontSize: 26 }}>↓{clinetInfo.dl_info_speed == null ? "0" : formatBytes(clinetInfo.dl_info_speed)}/s</Text>



</View>
        <ContentTitle title={''} style={{ color: 'black' }} />
        <Appbar.Action icon="plus" onPress={_handleMore} style={{ color: '#fff' }} />
      </Appbar.Header>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          <TouchableOpacity style={styles.row} onPress={() => onPress(item)} onLongPress={() => onPressLong(item.name)}>
            ...
            <ProgressBar style={{ height: 3, width: '100%', borderRadius: 20 }} progress={item.progress} />
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              {(() => {
                ...
                if (item.state == 'downloading') {
                  return (
                    <Text style={styles.markdown}>Downloading</Text>
                  )
                }
                if (item.state == 'metaDL') {
                  return (
                    <Text style={styles.markdown}>Checking Metadata</Text>
                  )
                }
